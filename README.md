...
This repository currently contains pinned build inputs and tested maintenance tools. Native build
recipes and distributable compiler artifacts are being prepared. There is no published compiler
package or artifact release yet. Passing repository checks does not establish device execution or
...
`mise run check` runs formatting, lint, source-inventory checks, Swift tests, and DocC in sequence.
...
maintenance commands. `Tests` contains focused failure and compatibility checks.

Compiler source checkouts, SDK contents, build directories, and release archives belong in ignored
storage. [Toolchain boundaries](Docs/Toolchain.md) describes the intended producer and artifact split.
